...
void  DynObjFilter::init(ros::NodeHandle& nh)
{
    ...
    nh.param<bool>("dyn_obj/input_in_world_frame", input_in_world_frame, false);
...
void  DynObjFilter::filter(PointCloudXYZI::Ptr feats_undistort, const M3D & rot_end, const V3D & pos_end, const double & scan_end_time)
{
    ...
    {   
        ...
        // If input is already in world frame (e.g., cloud_registered), skip transformation
        V3D p_glob = input_in_world_frame ? p_body : V3D(rot_end * (p_body) + pos_end);
        p[i].glob = p_glob;        
        p[i].dyn = STATIC;
