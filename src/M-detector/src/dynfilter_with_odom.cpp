...
void PointsCallback(const sensor_msgs::PointCloud2ConstPtr& msg_in)
{
    ...
    {
        if (field.name == "normal_x") has_normal_x = true;
        else if (field.name == "normal_y") has_normal_y = true;
        else if (field.name == "normal_z") has_normal_z = true;
        else if (field.name == "curvature") has_curvature = true;
    }

    if (has_normal_x && has_normal_y && has_normal_z && has_curvature)
    {
    ...
    else
    {
        ...
        pcl::fromROSMsg(*msg_in, cloud_xyzi);

        feats_undistort->clear();
        feats_undistort->reserve(cloud_xyzi.size());
        for (const auto &p_in : cloud_xyzi.points)
        {
            PointType p_out;
            p_out.x = p_in.x;
            p_out.y = p_in.y;
            ...
            p_out.normal_y = 0.0f;
            p_out.normal_z = 0.0f;
            p_out.curvature = 0.0f;
            feats_undistort->push_back(p_out);
        }
    }
