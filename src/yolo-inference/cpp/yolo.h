...
public:
	...
	/**
	 * @description: 				inference interface for in-memory images
	 * @param {Mat&} image		input image
	 * @param {bool} draw_result	whether to render overlays into m_result
	 * @return {*}
	 */
	void infer(const cv::Mat& image, bool draw_result = false);

	/**
	 * @description: get rendered result image
	 * @return {const cv::Mat&}
	 */
	const cv::Mat& result() const { return m_result; }
