...
YoloDetector::YoloDetector(
        ...
        float confThresh,
        int numClass
    ): trtFile_(trtFile), onnxFilePath_(trtFile), nmsThresh_(nmsThresh), confThresh_(confThresh), numClass_(numClass)
{
    gLogger = Logger(ILogger::Severity::kERROR);
    cudaSetDevice(gpuId);

    size_t extPos = onnxFilePath_.find_last_of('.');
    if (extPos == std::string::npos) {
        onnxFilePath_ += ".onnx";
    } else {
        onnxFilePath_.replace(extPos, std::string::npos, ".onnx");
    }
    if (access(onnxFilePath_.c_str(), F_OK) != 0) {
        onnxFilePath_ = onnxFile;
    }
...
void YoloDetector::get_engine(){
    if (access(trtFile_.c_str(), F_OK) == 0){
        ...
        if (!parser->parseFromFile(onnxFilePath_.c_str(), int(gLogger.reportableSeverity))){
