...
中心距:      d_center = ||P_static - O||
静态净空:    d_static = d_center - r_obj
```

含义：静态净空表示吊装目标外边界到静态环境最近点之间，还剩多少安全距离。
...
中心距:      ||p_dyn - p_obj||
动态净空:    d_dynamic = ||p_dyn - p_obj|| - r_obj - r_dyn
```

含义：动态净空表示当前时刻吊装目标与动态障碍物之间，按两者空间包络扣除后的剩余边界距离。
...
预测净空:
    d_pred = min_{tau in [0, H]} ||(p_dyn - p_obj) + (v_dyn - v_obj)tau|| - r_obj - r_dyn
```

含义：预测净空表示在未来预测时间窗内，两者最接近的那一刻，边界之间最小还会剩下多少距离。

### 4.4 三者区别总结

- 静态净空：现在与静态环境之间还剩多少安全边界。
- 动态净空：现在与动态目标之间还剩多少安全边界。
- 预测净空：未来最危险时刻还会剩下多少安全边界。

### 4.5 代码实现口径说明

- 当前静态净空基于 `map_local` 最近邻点计算。
- 当前动态净空和预测净空都采用目标包络半径近似，而不是精确多边形/包围盒求交。
- 当前代码里若净空计算结果小于 0，会被截断为 0，表示已经进入碰撞或重叠状态。
...
3. 补充说明静态净空、动态净空和预测净空分别对应“现在离静态障碍多远”“现在离动态障碍多远”“未来最危险时还有多远”。
4. 最后讲系统输出不仅有预警等级，还有控制接口原型输出。
